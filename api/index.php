<?php
// Comment the following two lines when we are online
...
foreach(array('method', 'auth_token', 'isbn', 'outdate', 'duedate', 'callback', 'firstname',
    'lastname', 'company', 'phone', 'email', 'address', 'city', 'state', 'postcode', 
    'userid', 'quantity', 'sortby', 'page', 'pageitems', 'catid') AS $var)
{
    $$var = '';
    if(isset($_GET[$var]))
    {
...
//end getbook
    ...
    $result = $oDB->select("SELECT catid,cat_name FROM category order by cat_name"); //mysql_query("SELECT * FROM books WHERE isbn10=7505715666 OR  isbn13 =7505715666 ");       

    if (mysqli_num_rows($result)) {
        $row_set = array();
        $i       = 0;
        while ($row = mysqli_fetch_assoc($result)) {
            $row_set[$i]['cat_name'] = urlencode($row['cat_name']);           
            $row_set[$i]['catid']    = $row['catid']; 
            $i++;
        }
    ...
    //$result = $oDB->select("SELECT * FROM books LEFT OUTER JOIN loan ON ( books.isbn10 = loan.isbn OR books.isbn13 = loan.isbn ) ");
     $sSql =  "SELECT books.*,category.*
                    FROM books
                    LEFT JOIN category ON books.catid = category.catid "  ;  
...
} else if ($method == 'getBooksByCategory') {
    //$result = $oDB->select("SELECT * FROM books LEFT OUTER JOIN loan ON ( books.isbn10 = loan.isbn OR books.isbn13 = loan.isbn ) ");
    $sSql =  "SELECT * FROM books LEFT JOIN category on books.catid = category.catid WHERE books.catid='$catid'"  ;  
    $row_set = array();
